    ...
    public static void calculateCurrencyConversion(Currency[] currencies) {
        for (Currency c : currencies) {
            if (c.getCurrency().equals("dollar")) {
                System.out.println("For the  " + c.getCurrency() + " user the monetary value is $: " + c.getValue() * Currency.VALUE_DOLLAR);
            } else if(c.getCurrency().equals("euro")){
                System.out.println("For the  " + c.getCurrency() + " user the  monetary value es $ : " + c.getValue() * Currency.VALUE_EURO);
            }else{
